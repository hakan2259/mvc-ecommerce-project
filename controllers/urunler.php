...
class urunler extends Controller
{
    ...
    {
        parent::UploadLibrary(array("View", "Bilgi", "Pagination"));
    ...
    function kategori($id, $ad, $currentPage = false)
    {

        $sonuc = $this->model->uruncek("urunler", "where katid=" . $id);
        $CocukKAtBul = $this->model->uruncek("alt_kategori", "where id=" . $id);
        //13
        $this->Pagination->PaginationCreate(count($sonuc), $currentPage,
            $this->model->SingleData("siteUrunlerGoruntuAdet", " from ayarlar"));
        ...
        if (count($sonuc) > 0 && count($CocukKAtBul) > 0):
            ...
            $this->View->goster("sayfalar/kategori",
                array(
                    "data1" => $this->model->uruncek("urunler", "where katid=" . $id . " LIMIT " . $this->Pagination->limit . "," . $this->Pagination->showNumber),
